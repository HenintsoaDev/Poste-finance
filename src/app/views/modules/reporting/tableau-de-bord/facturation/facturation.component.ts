...
export class FacturationComponent extends Translatable implements OnInit {
 ...
 async  filtreTableau()
  {
      ...
      let result = await this.authService.getSelectList(environment.facturation +  filtreParMulti,[]);

      this.data.facturation_HPM = result.facturation_HPM
      this.data.facturation_SUNUPAY = result.facturation_SUNUPAY
      this.data.recapitulatif_global = result.recapitulatif_global
      ...
      let title = this.__("dashboard.title_facturation") + ' ';
  ...
  get nbPair(): any[] { return this.data.facturation_SUNUPAY.filter((_, index) => index % 2 === 0); }
