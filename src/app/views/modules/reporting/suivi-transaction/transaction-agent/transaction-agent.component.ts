...
@Component({
  ...
  styleUrls: ['./transaction-agent.component.scss']
})
export class TransactionAgentComponent extends Translatable implements OnInit {
  ...
  listAgentActive = [];
  searchControlAgent = new FormControl('');
  agentId: number = null;
  agentLabel: string = "";
  filteredAgent = [];
  ...
  recupererAgent(event: MatSelectChange) {
    const idBureau = event.value;

    this.ressourceService.getListAgent(idBureau).subscribe({
      next: (res) => {
          if(res['code'] == 200) {
              console.log(res);
  ...
  this.searchControlAgent.valueChanges.subscribe(value => {
    const lower = value?.toLowerCase() || '';
    this.filteredAgent = this.listAgentActive.filter(agent =>
        agent.label.toLowerCase().includes(lower)
    );
});
  ...
  filtreTableau() {
      ...
      let agent = "";
      if(this.agentId) agent = "&agent=" + this.agentId;
