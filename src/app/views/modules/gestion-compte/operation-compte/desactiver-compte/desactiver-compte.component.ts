...
export class DesactiverCompteComponent extends Translatable implements OnInit {
  ...
  rechercheBeneficiaire(){
   ...
   desactiverCompte() {
    ...
    Swal.fire({
      ...
      text: this.__("global.desactiver_compte_?"),
      icon: 'warning',
      showCancelButton: true,
      confirmButtonText: this.__("global.oui_desactiver"),
      ...
      if (result.isConfirmed) {
           ...
           this.operationService.desactiverCompte(data).subscribe({
            next: (res) => {
                if(res['code'] == 201) {
